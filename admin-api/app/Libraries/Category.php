...
class Category {
     ...
     */
    private function _findAllCat($orderby = NULL) {
        $this->rawList = empty($orderby) ? $this->model->get() : $this->model->orderBy($orderby)->get();
     ...
     * @return  array            返回结构信息
     */
    public function getList($cid = 0, $orderby = NULL) {
        unset($this->rawList, $this->formatList);
	
        $this->_findAllCat($orderby, $orderby);		
