<template>
	<view class="w-100 flex-center flex-j-between flex-wrap">
		<view class="cont-list mb-12" v-for="(item,index) in data.good" :key="index" @click="goBuy(item)">
	...
	export default {
		...
		methods: {
			goBuy:function(item){
				...
				// 		self.app.showOpen('Shopping/commodity?data='+JSON.stringify(item));
				// 	}
				// });
				uni.redirectTo({
					url:"commodity?data="+JSON.stringify(item)
				});
			},
			getList:function(){
				var self=this;
