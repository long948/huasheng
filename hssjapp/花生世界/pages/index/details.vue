<template>
	<view class="box">
		...
		<view class="content pl-15 pr-15">
			...
			<view class="details w-100 newlines font-black pt-15 pb-15 lh-20">
...
<script>
	...
	export default {
		...
		methods: {
			...mapMutations(["setUserInfo","logout",'lang']),
			getData:function(id){
				var self=this;
				let url=config.api + "/notice-detail";
				uni.request({
					url: url,
					data: {
						Id:id
					...
					success: res => {
						...
						if(res.data.status==1){
							self.details=res.data.data.Content;
						};
					},
					fail: (res) => {
						console.log(JSON.stringify(res));
