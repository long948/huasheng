		...
		<view class="content">
			...
			<view class="form-cont">
				<view class="contInput flex-center">
					...
					<input type="text" :placeholder="lang.fgl1" v-model="mobile" maxlength="11" />
				</view>
				<view class="contInput flex-center pr-10">
					<input type="text" :placeholder="lang.fgl4" v-model="code" maxlength="6" />
					<!-- <button class="get-code-btn" :disabled="!code_status" v-if="!CodeSW" @click="sendCode()">{{lang.fgl2}}</button>
					<button class="get-code-btn" disabled v-else>{{second}}S{{lang.fgl3}}</button> -->
				...
				<view class="contInput flex-center">
					...
					<text class="split-line"></text>
					<input type="password" :placeholder="lang.fgl5" v-model="pass" maxlength="20" />
				</view>
				<view class="contInput flex-center">
					<image mode="widthFix" src="../../static/img/a50ae33f8439ac9db1b66a52c1e6e13.png"></image>
					<text class="split-line"></text>
					<input type="password" :placeholder="lang.fgl6" v-model="pass1" maxlength="20" />
				</view>
			</view>
			<view class="btn-cont pl-45 pr-45 pt-50">
				<button class="btn" @click="forgetLogin()" :disabled="!forget_status">{{lang.fgl7}}</button>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			forgetLogin:function(){
				...
				user.forgetLogin(send,function(res){
					if(res.data.status==1){
						self.app._toastIcon(self.lang.fgl8);
						setTimeout(function(){
							self.app.goBack();
						},300);
					}else{
			...
			initCode:function(){
				...
				Captcha.init(function(result) {
					...
					if (result.status == 1) {
						...
						self.code_type=2;
						sms.forgetSMS(send, function(res) {
							if(res.data.status==1){
								self.app._toastIcon(self.lang.fgl9);
								self.movetime();
								self.code_type=3;
							}else{
								self.code_type=1;
