	...
	<view class="box pt-15 pb-15 pl-15 pr-15">
		<view class="content bc-white br-4 pt-30 pb-30 pl-15 pr-15">
			<view class="flex-row flex-j-between cont-list">
				<view class="title one-row text-right">油卡选择</view>
				<view class="value newlines">
					<view class="nav nowrap" :class="{'active':(index+1) == ykindex}" v-for="(item,index) in ykList" :key="index" v-text="item" @click="ykindex=(index+1)">中国石油</view>
				</view>
			</view>
			<view class="flex-center flex-j-between cont-list">
				<view class="title one-row text-right">油卡卡号</view>
				<view class="value bt_line">
					<input type="text" v-model="id" />
				</view>
			</view>
			<view class="flex-center flex-j-between cont-list">
				<view class="title one-row text-right">所在地址</view>
				<view class="value bt_line">
			...
			<view class="flex-center flex-j-between cont-list">
				<view class="title one-row text-right">身份证号</view>
				<view class="value bt_line">
					<input type="text" v-model="card_id" />
				</view>
			</view>
			<view class="flex-center flex-j-between cont-list">
			...
			<view class="flex-row flex-j-between cont-list">
				...
				<view class="value newlines">
					<view class="nav nowrap" :class="{'active':index == moneyindex}" v-for="(item,index) in config.oil" :key="index" v-text="item+'元'" @click="moneyindex=index">100元</view>>
				</view>
			</view>
			...
			<view class="w-100 flex-center flex-j-between mt-20 pl-15 font-10" style="color: #666666;">
				...
				<view class="text-right font-10">预计扣除PT数量：<text class="font-main">{{ app._toFixed(app._accDiv(config.oil[moneyindex],config.price),2) }}</text></view>
			</view>
		</view>
		<view class="remarks pt-15 pl-15 pr-15">
			<view class="lh-25 newlines" v-text="config.oil_remarks"></view>
		</view>
...
<script>
	...
	export default {
		data() {
			return {
				ykList:["中国石化","中国石油"],
				config:{phone:[],oil:['---'],oil_remarks:"--",price:"--"},
				ykindex:1,
				moneyindex:0,
				id:"",
				address:"",
		...
		methods: {
			...
			Recharge:function(){
				...
				let send={
					...
					card_num:self.id,
					amount:self.config.oil[self.moneyindex],
