	...
	<view class="box pl-15 pr-15">
		<view class="cont-list pt-15 pb-15 bt_line" v-for="(item,index) in list" :key="index">
			<view class="flex-center flex-j-between font-16 font-black">
				<view class="max-w-50 nowrap" v-text="item.deduction_amount">-13.4524</view><view v-text="item.status">充值状态</view>
			</view>
			<view class="flex-center flex-j-between pt-10">
				<view class="font-12 font-cl-9" v-text="item.type+' '+item.amount">充值加油卡100</view>
	...
	export default {
		...
		methods: {
			...
			getList:function(){
				...
				let url=config.api + "/user-submit-ecology-order-list";
				uni.request({
					url: url,
					data: send,
					method: "get",
