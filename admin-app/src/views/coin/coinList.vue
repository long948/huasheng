<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="协议" align="center">
        <template slot-scope="scope">
          <span v-for="(item,index) in protocolList" :key="index">
            <span v-if="item.id == scope.row.Protocol">{{ item.name }}</span>
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否为平台币" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.is_platform == 0">否</span>
          <span v-if="scope.row.is_platform == 1">是</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="CoinAddVisible">
      <el-form>
        ...
        <el-form-item label="当前市场价格(单位USDT)" label-width="130px">
        ...
        <el-form-item label="提现手续费(1为100%)" label-width="93px">
          <el-input v-model="coin.WithDrawFee" />
        </el-form-item>
        <el-form-item label="最低提现手续费" label-width="93px">
        ...
        <el-form-item label="是否为平台币" label-width="130px">
          <el-radio v-model="coin.is_platform" label="1">是</el-radio>
          <el-radio v-model="coin.is_platform" label="0">否</el-radio>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    iconUpdateHandle(row) {
      ...
      getCoin(row.Id).then(res => {
        if (res.code === 20000) {
          ...
          this.coin.is_platform = res.data.is_platform ? res.data.is_platform : '0'
    ...
    handleAvatarSuccess(res, file) {
      ...
      this.coin.Logo = this.qiniu.Domain + res.key
    },
    // 添加修改币种
    handleAdd() {
